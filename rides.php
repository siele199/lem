...
$ride = "SELECT * FROM orderdeli INNER JOIN rider on rider.riderid=orderdeli.riderid INNER JOIN order1 on order1.id=orderdeli.orderid INNER JOIN users on users.username1=orderdeli.customer_user where order1.process='In Progress'AND order1.process='Completed'AND order1.process='Delayed' OR rider.username='$username' AND rider.available='1' AND order1.delivered='0'  ORDER BY `rider`.`username` ASC";
...
<html lang="en">
 ...
 <body>
    ...
    <li><a class="nav-link scrollto active" href="rides.php">Rides</a></li>
    <li><a class="nav-link scrollto" href="rrecords.php">Ride Records</a></li>
...
<td> <?php
    if ($row["released"] == 0) {
        echo '<p><a href="delivered.php?id=' . $row["username"] . '&released=1" class="btn btn-primary">Delivered</a></p>';
    }
    else {
        echo '<p><a href="delivered.php?id=' . $row["username"] . '" class="btn btn-warning">Awaiting Release</a></p>';
    }
?>
                </td>
